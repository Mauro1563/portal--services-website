...
export function Security() {
  ...
  return (
    <section id="security" className="relative border-y border-white/[0.06] bg-white/[0.03]/40 py-24">
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            {items.map(({ icon: Icon, title, sub }) => (
              <div key={title} className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-5">
                <Icon className="mb-3 h-5 w-5 text-cyan-300" />
                <p className="font-display text-sm font-semibold text-white">{title}</p>
                <p className="mt-1 text-xs leading-relaxed text-slate-300">{sub}</p>
              </div>
            ))}
          </div>
        </div>
