  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/[0.06] text-slate-200 hover:bg-white/[0.12] md:hidden"
      >
      ...
      {open ? (
        <>
          ...
          <button
            ...
            className="fixed inset-0 top-20 z-40 cursor-default bg-ink-0/60 backdrop-blur-sm md:hidden"
          />

          {/* Slide-down panel */}
          <div className="fixed inset-x-0 top-20 z-50 border-b border-white/[0.06] bg-ink-0/95 backdrop-blur-xl md:hidden">
            <nav className="mx-auto flex max-w-7xl flex-col gap-1 px-6 py-4">
              {links.map((l) => (
                <Link
                  ...
                  href={l.href}
                  onClick={() => setOpen(false)}
                  className="rounded-lg px-3 py-3 text-base font-medium text-slate-200 hover:bg-white/[0.04] hover:text-white"
                >
                  {l.label}
                </Link>
              ))}
              ...
              <div className="mt-3 border-t border-white/[0.06] pt-3">
                <Link
                  ...
                  onClick={() => setOpen(false)}
                  className="block rounded-lg px-3 py-3 text-base font-medium text-slate-200 hover:bg-white/[0.04] hover:text-white"
                >
                  {t('login')} →
                </Link>
              </div>
