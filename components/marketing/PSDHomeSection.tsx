 ...
 *
 * Residential cleaning + Airbnb Turnover Loop. Accent green #10B981
 * (Home CTA color per v2 brief). Screenshot placeholder is a dark
 * navy tile so the section reads consistently with the Workforce
 * card even though the surface bg is light.
 */
...
export default async function PSDHomeSection() {
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute -bottom-24 -left-24 h-72 w-72 rounded-full bg-[#10B981]/8 blur-3xl"
      />
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {pillars.map((key, i) => {
            ...
            return (
              <div
                key={key}
                className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-[#10B981]/40 hover:shadow-md"
              >
                <span className="grid h-10 w-10 place-items-center rounded-lg bg-[#10B981]/10 text-[#10B981]">
            ...
            {(['step1', 'step2', 'step3', 'step4'] as const).map(
              (stepKey, i, arr) => (
                <div key={stepKey} className="flex items-center gap-3">
                  <div className="inline-flex items-center gap-2 rounded-full border border-[#10B981]/30 bg-[#10B981]/5 px-3 py-1.5 text-xs font-semibold text-slate-800">
                    <span className="grid h-5 w-5 place-items-center rounded-full bg-[#10B981] text-[10px] font-bold text-white">
                      {i + 1}
                    </span>
                    {t(`loopSteps.${stepKey}`)}
                  </div>
